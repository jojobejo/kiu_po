...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `tb_detail_po` ADD COLUMN `harga_satuan_exclude` DECIMAL(18,4) NOT NULL DEFAULT 0 AFTER `hrg_satuan`',
    'SELECT "Column tb_detail_po.harga_satuan_exclude already exists"'
  )
...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `tb_detail_po` ADD COLUMN `harga_satuan_kecil_exclude` DECIMAL(18,4) NOT NULL DEFAULT 0 AFTER `harga_satuan_kecil`',
