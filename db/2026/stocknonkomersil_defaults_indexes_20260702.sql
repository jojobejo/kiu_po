...
SET @sql = (
  SELECT IF(
    ...
    'ALTER TABLE `tb_barang_nk` MODIFY COLUMN `minimum_stock` DECIMAL(18,2) NOT NULL DEFAULT 0'
  )
...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `tb_barang_nk` ADD INDEX `idx_barang_nk_kd_barang` (`kd_barang`)',
...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `tb_barang_nk` ADD INDEX `idx_barang_nk_kd_lokasi` (`kd_lokasi`)',
...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `tb_transaksi` ADD INDEX `idx_transaksi_barang_akun` (`kd_barang`, `kd_akun`)',
    'SELECT "Index idx_transaksi_barang_akun already exists"'
  )
  FROM INFORMATION_SCHEMA.STATISTICS
  WHERE TABLE_SCHEMA = @database_name
    AND TABLE_NAME = 'tb_transaksi'
...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `tb_transaksi` ADD INDEX `idx_transaksi_barang_tanggal` (`kd_barang`, `tgl_transaksi`(10))',
