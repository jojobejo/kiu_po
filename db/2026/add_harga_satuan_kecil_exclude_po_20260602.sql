...
SET @sql = (
  SELECT IF(
    COUNT(*) = 0,
    'ALTER TABLE `tbpo_detail_po` ADD COLUMN `harga_satuan_kecil_exclude` DECIMAL(18,4) NOT NULL DEFAULT 0.0000 AFTER `harga_satuan_kecil`',
...
UPDATE `tbpo_detail_po` d
LEFT JOIN `tbpo_po` p ON p.`kd_po` = d.`kd_po`
